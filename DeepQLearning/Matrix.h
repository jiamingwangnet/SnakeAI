...
namespace dqn
{
	...
	template<typename T>
	inline Matrix<T>::Matrix(std::vector<T> values, size_t rows, size_t columns, T init)
	...
	template<typename T>
	inline Matrix<T>::Matrix(size_t rows, size_t columns, T init)
	...
	template<typename T>
	inline const typename std::vector<T>::const_iterator Matrix<T>::begin() const
	{
		return values.begin();
	}

	template<typename T>
	inline const typename std::vector<T>::const_iterator Matrix<T>::end() const
	{
		return values.end();
	}
	...
	template<typename T>
	inline Matrix<T> Matrix<T>::operator*(const Matrix& rhs) const
	{
	...
	template<typename T>
	inline Matrix<T> Matrix<T>::operator*(const T& rhs) const
	{
	...
	template<typename T>
	inline Matrix<T> Matrix<T>::Hadamard(const Matrix& rhs) const
	{
		assert(SizeEqu(rhs));
		Matrix res{ rows, columns };
		for (size_t i = 0; i < rows * columns; i++)
	...
	template<typename T>
	inline Matrix<T> Matrix<T>::GetTransposed() const
	{
	...
	template<typename T>
	inline bool Matrix<T>::SizeEqu(const Matrix& other) const
	{
